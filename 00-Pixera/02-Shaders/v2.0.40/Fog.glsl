...
struct Fog {
  ...
  float strength;
  //@ label: "Red", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, raange_default: 255, group: rgbColorPicker, group_label: "Color"
  float colorR;
  //@ label: "Green", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, raange_default: 255, group: rgbColorPicker
  float colorG;
...
vec4 texture(Fog s, vec2 tex_coords) {
  ...
  vec4 color = vec4(s.colorR, s.colorG, s.colorB, orig.a);
