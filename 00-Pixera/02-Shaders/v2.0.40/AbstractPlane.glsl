...
struct AbstractPlane{
	...
	float time;
	//@ label: "Foreground Red", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 255, group: rgbColorPicker, group_label: "Foreground", partInfo: 1-2
	float fColorR;
	//@ label: "Foreground Green", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 0, group: rgbColorPicker
	float fColorG;
	//@ label: "Foreground Blue", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 255, group: rgbColorPicker
	float fColorB;
	//@ label: "Background Red", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 76, group: rgbColorPicker, group_label: "Background", partInfo: 2-2
	float bColorR;
	//@ label: "Background Green", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 0, group: rgbColorPicker
	float bColorG;
	//@ label: "Background Blue", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 255, group: rgbColorPicker
	float bColorB;
};
...
vec3 cellTileColor(in vec3 p){
    ...
    vec3 col = col1.rgb;//vec3(.7, .8, 1);
    
   
    if (cellID == 3) col = col2.rgb;
...
vec4 texture( AbstractPlane s, vec2 tex_coords ){
  vec4 orig = texture(s.sampler, tex_coords);
	float iTime = s.time;
	col1 = vec3(s.fColorR, s.fColorG, s.fColorB)*10.0;
	col2 = vec3(s.bColorR, s.bColorG, s.bColorB)*10.0;
	// Screen coordinates.
	vec2 uv = (tex_coords - iResolution.xy*.5)/iResolution.y;
	...
	*/ 
	...
	if(t < FAR){
	    ...
	    //sn = texBump(s.sampler, sp*tSize0, sn, 0.01);
     
        // Obtaining the texel color. 
	    //vec3 texCol = tex3D(s.sampler, sp*tSize0, sn);
		vec3 texCol = vec3(0.3);
	    // Ambient occlusion.
	    ...
	    float shading =  0.5; //smoothstep(-.05, .1, cellTile(sp));//
        ...
        sceneCol += (rCol*(diff + ambience) + vec3(.8, .95, 1)*spec*fak + env)*atten;
        sceneCol += (rCol*(diff2 + ambience) + vec3(.8, .95, 1)*spec2*fak + env)*atten2;
	...
	return mix(orig, vec4(sqrt(clamp(sceneCol, 0., 1.)), orig.a), s.mix);
