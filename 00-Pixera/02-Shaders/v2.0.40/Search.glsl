...
float movingLine(vec2 uv, vec2 center, float radius, float time)
{
    //angle of the line
...
float bip2(vec2 uv, vec2 center, float time)
{
    float r = length(uv - center);
...
vec4 texture(Search s, vec2 tex_coords) {
 ...
 vec3 finalColor;
    ...
    finalColor += triangles(uv, c, 315.0 + 30.0*sin(s.time)) * blue2;
    finalColor += movingLine(uv, c, 240.0, s.time) * blue3;
    finalColor += circle(uv, c, 10.0, 1.0) * blue3;
    finalColor += 0.7 * circle2(uv, c, 262.0, 1.0, 0.5+0.2*cos(s.time)) * blue3;
    if( length(uv-c) < 240.0 )
    {
        //animate some bips with random movements
    	vec2 p = 130.0*MOV(1.3,1.0,1.0,1.4,3.0+0.1*s.time);
   		finalColor += bip1(uv, c+p) * vec3(1,1,1);
        p = 130.0*MOV(0.9,-1.1,1.7,0.8,-2.0+sin(0.1*s.time)+0.15*s.time);
        finalColor += bip1(uv, c+p) * vec3(1,1,1);
        p = 50.0*MOV(1.54,1.7,1.37,1.8,sin(0.1*s.time+7.0)+0.2*s.time);
        finalColor += bip2(uv,c+p, s.time) * red;
    }
	
	return mix(orig, vec4( finalColor, orig.a ), s.mix);
}
