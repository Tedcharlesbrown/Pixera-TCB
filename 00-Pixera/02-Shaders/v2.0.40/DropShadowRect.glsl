...
struct DropShadowRect {
  ...
  float height;
  //@ label: "ShadowWidth[%]", editor: range, min: 0, max: 0.5, range_min: 0, range_max: 50, range_default: 0
  float shadowWidth;
...
vec4 texture(DropShadowRect s, vec2 tex_coords) {
  ...
  if(aspectRatio >= 1. && (tex_coords.x <= diff || tex_coords.x > 1. - diff)
    ...
    return mix(orig, vec4(shadowColor, 0), s.mix);
  if(isOutsideBorder(tex_coords, s.shadowWidth)) {
  	...
  	alpha = alpha < orig.a ? alpha : orig.a;
  	return mix(orig, vec4(shadowColor, alpha), s.mix);
  }
  tex_coords = (tex_coords - s.shadowWidth) / (1. - 2. * s.shadowWidth); // scaling
  vec4 result = mix(texture(s.sampler, tex_coords), orig, 1-s.mix);
  result.a = orig.a;
  return result;
}
