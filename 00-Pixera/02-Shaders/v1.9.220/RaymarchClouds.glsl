...
struct RaymarchClouds{
	...
	float fColorB;
	//@ label: "Background Red", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, range_default: 0, group: rgbColorPicker, group_label: "Background", partInfo: 2-2
	float bColorR;
...
vec4 texture(RaymarchClouds s, vec2 tex_coords)
{
  vec4 orig = texture(s.sampler, tex_coords);
    ...
    return mix(texture(s.sampler, tex_coords), vec4(col.rgb*bCorrection, orig.a), s.mix);
}
