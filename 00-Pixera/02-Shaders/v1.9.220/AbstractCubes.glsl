...
struct AbstractCubes{
	...
	float gN;
	//@ label: "Red", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, raange_default: 200, group: rgbColorPicker, group_label: "Color of Cubes"
  	float fColorR;
  	//@ label: "Green", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, raange_default: 200, group: rgbColorPicker
  	float fColorG;
  	//@ label: "Blue", editor: range, min: 0, max: 1, range_min: 0, range_max: 255, raange_default: 255, group: rgbColorPicker
  	float fColorB;
...
vec4 texture(AbstractCubes s, vec2 tex_coords)
{
  vec4 orig = texture(s.sampler, tex_coords);
	...
	return mix(orig, vec4(render( ro, rd )*gCol, orig.a), s.mix);
}